  ...
  $.fn.saleItemsInit = function() {
	  ...
	  $(viewList).datagrid({
			...
			columns:[[
				...
				{field:'saleItemName',title:'消费项目名称',width:100,align:"center"},
	...
	var setValue = function(){
		var saleItemName = $.trim($('#saleItemName',editForm).val());
		if(''==saleItemName){
			$.messager.alert('提示','请填写消费项目名称','warning');
			return false;
		}
	...
	//查询
	var search = function(){
		var saleItemNameSearch = $('#saleItemNameSearch',queryContent).val();
		var content = {saleItemName:saleItemNameSearch};
		$(viewList).datagrid({
			queryParams:content
		});
		checkBtnRight();
	...
	var onMulDelete = function(){
		...
		$.messager.confirm("提示！","确定要删除选中的记录?",function(t){ 
			...
			for(var i=0;i<rows.length;i++){
				idArray.push(rows[i].saleItemId);
			}
			var ids = idArray.join(CSIT.join);
			var url = "dict/mulDeleteSaleItem.do";
			var content = {ids:ids};
			$.post(url,content,
				function(result){
					if(result.isSuccess){
